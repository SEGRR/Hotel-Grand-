...
<html lang="en">
              ...
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                  ...
                  <?php
                     ...
                     if(isset($_POST['login']) && isset($_POST['password']) && isset($_POST['email']) )
                     {
                        ...
                        ?>
                           <script>location.replace("booking.php");</script>
                        <?php
  ...
  <p class="fs-5 fw-bolder">Filters</p>
            <p>Room type</p>
            <form action="booking.php" method="get">
              <div class="form-check">
                <input class="form-check-input" type="checkbox" value="standard" id="flexCheckDefault" name="filters[]" <?php if( isset($_GET['filters']) && in_array('standard',$_GET['filters'])){ echo "checked";} ?>>
                <label class="form-check-label" for="flexCheckDefault">
                  Standard Room
                   <p class="fw-lighter">- AC,TV,Double Bed/Single Bed</p>
                </label>
              </div>
              <div class="form-check">
                <input class="form-check-input" type="checkbox" value="budget" id="flexCheckDefault" name="filters[]"  <?php if( isset($_GET['filters']) && in_array('budget',$_GET['filters'])){ echo "checked";} ?>>
                <label class="form-check-label" for="flexCheckDefault">
                  Budget Room
                  <p class="fw-lighter">- Non-AC,TV,Double Bed/Single Bed</p>
                </label>
              </div>
              <div class="form-check">
                <input class="form-check-input" type="checkbox" value="luxery" id="flexCheckDefault" name="filters[]" <?php if( isset($_GET['filters']) && in_array('luxery',$_GET['filters'])){ echo "checked";} ?>>
                <label class="form-check-label" for="flexCheckDefault">
                  Luxery Room
                  <p class="fw-lighter">- AC,Double bed,Living room,free Lunch/Dinner</p>
                </label>
              </div>
              <button  class="btn btn-danger p-1" type="button"  name="reset" value="Clear FIlter" >
                 <a href="booking.php" class="text-decoration-none text-light"> Clear Filter </a>
                 </button>
              <input class="btn btn-primary  float-end" type="submit" name="filter" value="Search">
  ...
  <div class="card card-body m-2 ">
            ...
            <p class="fs-5 fw-bolder">Sort</p>
             <form action="" method="get">
             <select class="form-select" name="sort" aria-label="Default select example">
                      <option>--- Select One Option ---</option>
                      <option value="1"  <?php if(isset($_GET['sort']) && $_GET['sort'] == 1 ){ echo 'selected';} ?>  >Sort by Price Ascending</option>
                      <option value="2"   <?php if(isset($_GET['sort']) && $_GET['sort'] == 2 ){ echo 'selected';} ?>>Sort by Price Desceding</option>
                      <option value="3"    <?php if(isset($_GET['sort']) && $_GET['sort'] == 3 ){ echo 'selected';} ?>>Sort by Availibility</option>
                    </select>
                    <button  class="btn btn-danger m-2" type="button"  name="reset" value="Clear FIlter" >
                 <a href="booking.php" class="text-decoration-none text-light"> Reset </a>
                 </button>
                    <input class="btn btn-primary  float-end m-2" type="submit" name="sorter" value="Search" >     
             </form>
         ...
         <!--main content-->
         <div class="col-md-8 bg-light mt-3 p-1" style="overflow: scroll; overflow-x:hidden; height: 700px;">
         ...
         <?php
              ...
              if(isset($_GET['sorter'])){
                ...
                if($_GET['sort'] == 1){
                  ...
                  $sort_by= " ORDER BY available DESC ";
                }
             ...
             if(isset($_GET['filters'])){
              ...
              $chekedfilters =  $_GET['filters'];
              

             
                
                foreach($chekedfilters as $filter){
                       
                  $query = "SELECT * FROM `rooms` WHERE room_type = '$filter' $sort_by";
                  $rooms = mysqli_query($conn,$query) or die(mysqli_query($conn));
                  
                       
                while($arr= mysqli_fetch_assoc($rooms)){

                  extract($arr);
                  $dicountedPrice = ($arr['price'] * $arr['discount']) / 100;
                  $finalprice = $arr['price'] -  $dicountedPrice ;

               ?>
               
               <div class="col">
                <a href="<?php echo 'room.php?id='.$arr['room_id'];?>" class="text-decoration-none text-black" >
           <div class="card m-3" style="max-width: 540px;" >
                 <img src="data:image/jpg;charset=utf8;base64,<?php echo base64_encode($arr['photo_1']); ?>" width="500" height="300" class="card-img-top" alt="room photo">
                 <div class="card-body">
                   <h5 class="card-title "><?php echo $arr['room_name'];?> 
                  
                   <div class="col-3 float-end">
                      <p class=" text-warning">Rs.<?php echo $finalprice; ?></p>
                        <small ><p class="text-decoration-line-through ">Rs.<?php echo $arr['price'] ?> </p> <p class="text-success"><?php echo $arr['discount'];?>% off</p> </small>
                  </div>  
                  </h5>
                   <p class="card-text">
                     Room Type : <?php echo $arr['room_type']; ?> 
                     <ul>
                      <li><?php echo $arr['ac']; ?> Air conditon</li>
                     <li><?php echo $arr['bed_type']; ?></li> 
                     <li> 1 Bathroom</li>
                     <li>24 hour Hot water</li> 
                     </ul>
                   </p>
                   <p class="card-text"><small class="text-muted"><?php echo $arr['available']; ?> Rooms available</small>  </p>
                   <div>
                   <?php
                       for ($i=1; $i <= $arr['ratings']; $i++) { 
                         # code...
                   ?>
             ...
             $query = "SELECT * FROM `rooms` $sort_by";
